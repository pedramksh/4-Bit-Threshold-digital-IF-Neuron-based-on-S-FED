def write_to_file(filename):
    ...
    vwell_values = [round(-0.4 + i * 0.1, 1) for i in range(0 , 21)]  # Sweep vwell from -0.4 to 1.6 in steps of 0.1

    with open(filename, 'w') as file:
        for well_index, vwell in enumerate(vwell_values):
            for drain_index, vdrain in enumerate(vdrain_values):
                mode_id = f"{well_index:02}_{drain_index:02}"
                file.write(f"#*****************ID VD Mode {mode_id} ************************************* \n")
                file.write("solve        vgate=0\n")
                ...
                file.write("solve        name=gate vfinal=-0.5 vstep=-0.05 qscv\n")
                file.write(f"log          outf=sfedMode{mode_id}.log  \n")
                file.write("solve        name=gate vfinal=1.6 vstep=0.05 qscv\n")
                file.write("log off\n")

# Specify the filename and write the content
def main():
    output_file = "C:/Users/USER/Desktop/درس مرس/ارشد/ترم 2/PROJECT/S-FED DR_capG/sweep_capacitance.txt"
